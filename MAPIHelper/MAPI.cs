...
namespace MAPIHelper
{
    public class MAPI
    {
        ...
        int SendMail(string strSubject, string strBody, int how)
        {
            ...
            var curdir = Directory.GetCurrentDirectory(); // сохраняем текущий рабочий каталог потому, что Outlook Express меняет его на свой
            m_lastError = MAPISendMail(IntPtr.Zero, IntPtr.Zero, msg, how, 0);
            Directory.SetCurrentDirectory(curdir); // восстанавливаем рабочий каталог после возможной его замены почтовой программой
        ...
        public string GetError(int _code)
        {
            if (_code < 0)
                return "MAPI error [" + _code.ToString() + "] (возможно не установлен обработчик SimpleMAPI)" ;
            if (_code <= 26)
                return errors[_code];
            return "MAPI error [" + _code.ToString() + "]";
        }
